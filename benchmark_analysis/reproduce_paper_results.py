...
LONG_FORM_RUNNERS = {"figure2", "compact_grid", "compact_severe", "uncertainty", "mice_vs_coxph", "baseline_severe_all"}
SUMMARY_RUNNERS = {"summary_only", "realdata_summary"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--audit-inputs", action="store_true", help="Check required local input artifacts for the selected target without generating paper outputs.")
    ...
    parser.add_argument("--realdata-root", type=Path, default=None, help="Reserved for local credentialed real-data summary roots; summary targets should pass explicit --input files.")
...
def _audit_long_form_input(args: argparse.Namespace, target) -> dict[str, Any]:
    input_path, df = load_long_form_results(args, target.target_id)
    seed_count = int(df["cohort_seed"].nunique()) if "cohort_seed" in df.columns else 0
    return {
        "target_id": target.target_id,
        "status": "ok",
        "input_path": str(input_path),
        "rows": int(len(df)),
        "columns_checked": "scenario, model, cohort_seed, rep, p, K, ibs, auc",
        "details": f"normalized long-form stress table; n_cohort_seeds={seed_count}",
    }


def _audit_scenario_dataset_input(args: argparse.Namespace, target) -> dict[str, Any]:
    if args.input is not None:
        ...
        return {
            ...
            "input_path": str(input_path),
            "rows": int(len(df)),
            "columns_checked": "precomputed scenario diagnostics table",
            "details": "readable precomputed summary",
        }
    if args.dataset_root is None:
        raise ReproductionInputError(f"{target.target_id} requires local SynLV split files via --dataset-root or a precomputed diagnostics table via --input.")
    dataset_root = args.dataset_root.expanduser().resolve()
    if not dataset_root.exists():
        raise ReproductionInputError(f"Dataset root for {target.target_id} does not exist: {dataset_root}")
    missing: list[str] = []
    checked = 0
    ...
    if missing:
        ...
        raise ReproductionInputError(f"{target.target_id} is missing required split files: {preview}")
    return {
        "target_id": target.target_id,
        "status": "ok",
        "input_path": str(dataset_root),
        "rows": checked,
        "columns_checked": "split-file presence",
        "details": f"found {checked} train/val/test split files across registry scenarios and cohort seeds",
    }


def _audit_summary_input(args: argparse.Namespace, target) -> dict[str, Any]:
    if args.input is None:
        input_kind = "credentialed local real-data summary" if target.runner == "realdata_summary" else "local/precomputed result summary"
        raise ReproductionInputError(f"{target.target_id} requires --input pointing to a {input_kind}. The public code repository does not bundle this table.")
    input_path = args.input.expanduser().resolve()
    df = read_table(input_path)
    return {
        "target_id": target.target_id,
        ...
        "input_path": str(input_path),
        "rows": int(len(df)),
        "columns_checked": "table readable",
        "details": f"readable summary table with {len(df.columns)} columns",
    }


def audit_target_inputs(args: argparse.Namespace, target_name: str) -> dict[str, Any]:
    target = resolve_target(target_name)
    if target.runner in LONG_FORM_RUNNERS:
        return _audit_long_form_input(args, target)
    if target.runner == "scenario_mechanism":
        return _audit_scenario_dataset_input(args, target)
    if target.runner in SUMMARY_RUNNERS:
        return _audit_summary_input(args, target)
    raise KeyError(f"No audit checker registered for {target.target_id}: {target.runner}")


def print_audit_results(results: list[dict[str, Any]], errors: list[str]) -> None:
    if results:
        display = pd.DataFrame(results)
        print(display[["target_id", "status", "input_path", "rows", "columns_checked", "details"]].to_string(index=False))
    for error in errors:
...
def _scenario_cohort_dir(dataset_root: Path, scenario_key: str, cohort_seed: int) -> Path:
    ...
    cohort = f"cohortseed_{int(cohort_seed):03d}"
    candidates = [
        dataset_root / spec.internal_source_root / cohort,
        dataset_root / scenario_key / cohort,
        dataset_root / Path(spec.internal_source_root).name / cohort,
    ]
...
def run_summary_wrapper(args: argparse.Namespace, target, realdata: bool = False) -> dict[str, Any]:
    if args.input is None:
        source = "credentialed local real-data summary" if realdata else "precomputed synthetic summary"
        raise TargetUnavailable(f"{target.target_id} is a summary/inferential target. Pass --input with the {source} table. The public repository does not bundle this result artifact.")
    input_path = args.input.expanduser().resolve()
    df = read_table(input_path)
    tex = simple_latex_table(df, caption=target.caption, label=target.latex_label)
...
def main() -> None:
    ...
    if args.audit_inputs:
        selected = [target.target_id for target in TARGETS] if args.target in {"", "all"} else [args.target]
        results: list[dict[str, Any]] = []
        errors: list[str] = []
        for name in selected:
            try:
                results.append(audit_target_inputs(args, name))
            except (TargetUnavailable, ReproductionInputError, ValueError, KeyError) as exc:
                errors.append(f"{name}: {exc}")
        print_audit_results(results, errors)
        if errors:
            raise SystemExit(2)
        return
    if not args.target:
        raise SystemExit("Pass --list-targets, --self-test, --audit-inputs, or --target <target_id|label|all>.")
